...
from speechmarkdown.formatters.data.amazon_polly_voices import AMAZON_POLLY_NEURAL_VOICES
...
class AmazonPollyNeuralSsmlFormatter(SsmlFormatterBase):
    def __init__(self, options: SpeechOptions):
        ...
        self.valid_voices = AMAZON_POLLY_NEURAL_VOICES
    ...
    def get_text_modifier_object(self, ast: ASTNode) -> TagsObject:
        ...
        for child in ast.children:
            if child.name in ('plainText', 'plainTextSpecialChars', 'plainTextEmphasis', 'plainTextPhone', 'plainTextModifier'):
                ...
                if key in ('address', 'cardinal', 'characters', 'digits', 'expletive', 'fraction', 'number', 'ordinal', 'telephone', 'unit'):
        ...
        for child in ast.children:
            if child.name == 'sectionModifierKeyOptionalValue':
                ...
                if key == 'lang':
                    ...
                    so.voice_tag(value)
                elif key in ('dj', 'excited', 'disappointed'):
                    pass
