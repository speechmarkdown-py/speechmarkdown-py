...
class TagsObject:
    def __init__(self, base: 'SsmlFormatterBase'):
        self.base = base
        self.tags = {}
        self.text = ""

    def tag(self, tag_name: str, attrs: dict, augment: bool = False):
        sort_id = self.base.ssml_tag_sort_order.index(tag_name) if tag_name in self.base.ssml_tag_sort_order else 999
        if tag_name not in self.tags:
            self.tags[tag_name] = {"sortId": sort_id, "attrs": None}
        
        if augment:
            current_attrs = self.tags[tag_name]["attrs"] or {}
            self.tags[tag_name]["attrs"] = {**current_attrs, **attrs}
        else:
            self.tags[tag_name]["attrs"] = attrs

    def voice_tag_named(self, voices: Optional[dict], name: str) -> bool:
        if voices and name in voices:
            info = voices[name]
            if not isinstance(info, dict):
                info = {"voice": {"name": name}}
            
            metadata_keys = ["id", "displayName", "languages", "language", "locale", "isHD"]
...
class SsmlFormatterBase(FormatterBase):
    ...
    def __init__(self, options: SpeechOptions):
        ...
        self.modifier_key_mappings = {
            'chars': 'characters', 'cardinal': 'number', 'digits': 'characters',
            'bleep': 'expletive', 'phone': 'telephone', 'vol': 'volume'
        }

        self.ssml_tag_sort_order = [
            'emphasis', 'say-as', 'prosody', 'amazon:domain', 'amazon:effect',
            'amazon:emotion', 'voice', 'lang', 'sub', 'phoneme'
        ]

        self.modifier_key_to_ssml_tag_mappings = {
            'emphasis': 'emphasis', 'address': 'say-as', 'number': 'say-as',
            'characters': 'say-as', 'expletive': 'say-as', 'fraction': 'say-as',
            'interjection': 'say-as', 'ordinal': 'say-as', 'telephone': 'say-as',
            'unit': 'say-as', 'time': 'say-as', 'date': 'say-as', 'whisper': None,
            'sub': 'sub', 'ipa': 'phoneme', 'rate': 'prosody', 'pitch': 'prosody',
            'volume': 'prosody', 'drc': None, 'timbre': None, 'lang': None,
            'voice': None, 'dj': None, 'defaults': None, 'newscaster': None,
            'excited': None, 'disappointed': None
        }

    def format(self, ast: Any) -> str:
        lines = self.formatFromAst(ast, [])
        return "".join(lines)

    def add_section_start_tag(self, tags_sorted_asc: List[str], so: TagsObject, lines: List[str]):
    ...
    def add_section_end_tag(self, lines: List[str]):
        if self.section_tags:
            for tag in self.section_tags:
                lines.append(self.end_tag(tag, False))
                lines.append('\n')

    def add_tag(self, tag: str, ast: ASTNode, new_line: bool, new_line_after_end: bool, attr: dict, lines: List[str]) -> List[str]:
    ...
    def apply_tags_object(self, tmo: TagsObject, lines: List[str]) -> List[str]:
        tags_sorted_desc = sorted(tmo.tags.keys(), key=lambda t: tmo.tags[t]["sortId"], reverse=True)
        inner = tmo.text
    ...
    def get_short_ipa_object(self, ast: ASTNode, fallback_text: str = "") -> TagsObject:
        ...
        extracted_text = self.extract_parenthesized_text(text_node) if text_node and text_node.name == 'parenthesized' else getattr(text_node, 'allText', '')
    ...
    def get_short_sub_object(self, ast: ASTNode) -> TagsObject:
        tmo = TagsObject(self)
        text_node = next((c for c in ast.children if c.name in ('parenthesized', 'plainTextModifier')), None)
        tmo.text = self.extract_parenthesized_text(text_node) if text_node and text_node.name == 'parenthesized' else getattr(text_node, 'allText', '')
    ...
    def add_speak_tag(self, ast: Any, new_line: bool, new_line_after_end: bool, attr: dict, lines: List[str]) -> List[str]:
    ...
    def start_tag(self, tag: str, attr: dict, new_line: bool = False) -> str:
        attr_str = ''
        if attr:
    ...
    def void_tag(self, tag: str, attr: dict) -> str:
        attr_str = ''
        if attr:
            attr_str = ' ' + ' '.join(f'{k}="{v}"' for k, v in attr.items())
        return f"<{tag}{attr_str}/>"

    def add_tag_with_attrs(self, lines: List[str], text: str, tag: str, attrs: dict, force_end_tag: bool = False) -> List[str]:
    ...
    def get_tag_with_attrs(self, text: str, tag: str, attrs: dict) -> str:
        if text:
    ...
    def sentence_case(self, text: str) -> str:
        ...
        return re.sub(r'[a-z]', replacer, text, count=1).strip()

    def escape_xml_characters(self, unescaped: str) -> str:
        rev_pattern = "|".join(re.escape(k) for k in self.XML_UNESCAPE_MAPPING.keys())
        reversed_str = re.sub(rev_pattern, lambda m: self.XML_UNESCAPE_MAPPING[m.group(0)], unescaped)
        pattern = "".join(re.escape(k) for k in self.XML_ESCAPE_MAPPING.keys())
        escaped = re.sub(f"[{pattern}]", lambda m: self.XML_ESCAPE_MAPPING[m.group(0)], reversed_str)
        return escaped

    def get_voice_tag_fallback(self, name: str) -> Optional[dict]:
        return None
